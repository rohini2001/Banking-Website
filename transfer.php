...
{
    $sname=$_POST['customer'];
$q3="SELECT * FROM customer where cid=$sname";
   ...
   // echo $amount;
    if($amount > $sbal){
      ...
      echo $amount;
        ...
        echo "<script type='text/javascript'>alert('Can not transfer to self!!');</script>";
    }
     //echo $rname;
     else{
  ...
  $rbal=$qr2['balance'];
      ...
      //  echo "sbalance ".$sbal;
       // echo "rbalance ".$rbal;
        $rbal=$rbal+$amount;
$sbal=$sbal-$amount;
        $a1 = mysqli_query($conn,"UPDATE customer SET balance=$sbal WHERE cid=$sname");
        $a2 = mysqli_query($conn,"UPDATE customer SET balance=$rbal WHERE cid=$rname");
        $a3 = mysqli_query($conn,"INSERT INTO trans(sname,rname,amount) VALUES($sname,$rname,$amount)");
...
<label for="reciver">Choose a Receiver:</label><br>
<select name="reciver"class="form-control"  id="customer">
<?php
...
?>
  ...
  <?php } ?>
              ...
              <div class="form-group col-sm-8">
                ...
                <label for="val">Amount:</label>
<?php echo $GLOBALS['sbal'];?>
<input type="number"class="form-control" placeholder="Amount"   name="val" id="val" >
